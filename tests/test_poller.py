...
from sentrix.models.alert import Alert, AlertType, AlertSeverity
...
def poller(mock_config):
    with patch("sentrix.core.poller.InjectiveClient", autospec=True) as MockInjective, \
         patch("sentrix.core.poller.LLMClient", autospec=True) as MockLLM, \
         patch("sentrix.core.poller.RiskDetector", autospec=True) as MockDetector, \
         patch("sentrix.core.poller.RiskAnalyzer", autospec=True) as MockAnalyzer:
        
        p = Poller(mock_config)
        p.injective = MockInjective.return_value
        p.llm = MockLLM.return_value
        p.detector = MockDetector.return_value
        p.analyzer = MockAnalyzer.return_value
